...
dlptr insertatbeggining(dlptr head,int data)
{
	dlptr temp=new struct node();
	temp->data=data;
	temp->pnode=NULL;
...
dlptr specificpos(dlptr head,int x,int kd)//kd=knowndata
{
	dlptr temp=new struct node();
	dlptr l=head;
	temp->data=x;
...
int main()
{
	...
	cout<<endl;
	cin>>beg;
	head=insertatbeggining(head,beg);
	showlist(head);
	cout<<"enter knowndata: ";
	cin>>kd;
	cout<<"enter data:";
	cin>>d;
	head=specificpos(head,d,kd);
	showlist(head);
